...
def main(_):
    ...
    if accelerator.distributed_type == "DEEPSPEED":
        ...
        # Get values from your ml_collections config
        micro_batch_size = config.sample.train_batch_size
        grad_accum_steps = config.train.gradient_accumulation_steps
    
        # Inject values into the DeepSpeed config dictionary
        state.deepspeed_plugin.deepspeed_config["train_micro_batch_size_per_gpu"] = micro_batch_size
        state.deepspeed_plugin.deepspeed_config["gradient_accumulation_steps"] = grad_accum_steps
    
        logger.info(f"Injected DeepSpeed config: train_micro_batch_size_per_gpu={micro_batch_size}")
        logger.info(f"Injected DeepSpeed config: gradient_accumulation_steps={grad_accum_steps}")
    if accelerator.is_main_process:
        wandb.init(
            project="flow_grpo",
        )
